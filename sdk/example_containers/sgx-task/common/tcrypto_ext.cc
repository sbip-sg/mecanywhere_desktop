...
// for production, different value should be used for encryption (need to support update and thread synchronization on them)
...
sgx_status_t decrypt_content_with_key_aes(const uint8_t* src,
  ...
  sgx_status_t ret = (result == NULL) 
    ? SGX_ERROR_OUT_OF_MEMORY : SGX_SUCCESS;
  ...
  if (ret == SGX_SUCCESS) {
    ...
    ret = sgx_rijndael128GCM_decrypt(
      ...
      (uint32_t) cipher_text_size, result, iv_buf, AES_GCM_IV_SIZE,
      aad_buf, AES_GCM_AAD_SIZE, (const sgx_aes_gcm_128bit_tag_t*) tag_buf);
...
sgx_status_t encrypt_content_with_key_aes(const uint8_t* src,
  ...
  sgx_status_t ret = sgx_rijndael128GCM_encrypt(
    (const sgx_aes_gcm_128bit_key_t*) encryption_key, src,
    (uint32_t) size, whole_cipher_text, enc_iv_buf, AES_GCM_IV_SIZE,
    enc_aad_buf, AES_GCM_AAD_SIZE, (sgx_aes_gcm_128bit_tag_t*) tag_buf);
