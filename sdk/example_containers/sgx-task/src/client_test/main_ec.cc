...
unsigned char default_output_key[16] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37,
...
std::string get_public_key_as_string(EVP_PKEY* pkey) {
  ...
  BIO_free(bio);
  auto ret = std::string((char*)local_public_key, strlen((char*)local_public_key));
...
void load_private_key(const char* pri_key, size_t pri_key_size, EVP_PKEY** pkey) {
...
std::string encrypt_request(const std::string& data, const std::string& enclave_public_key, const std::string& output_key) {
  ...
  auto session_key = kdf_derive((const unsigned char*)secret.c_str(), secret.size(), "info", 4, 16);

  // encrypt data with the derived secret
  EVP_CIPHER_CTX* ctx2 = EVP_CIPHER_CTX_new();
  if (EVP_EncryptInit_ex(ctx2, EVP_aes_128_gcm(), NULL, (const unsigned char*) session_key.c_str(), iv) != 1) {
  ...
  int len;
  if (EVP_EncryptUpdate(ctx2, encrypted, &len, (const unsigned char*) plaintext.data(), plaintext.size()) != 1) {
  ...
  printf("tmp ec key pair (len: %ld) : %s\n", tmp_ec_key_pair_str.size(), tmp_ec_key_pair_str.c_str());

  EVP_PKEY_free(pkey);
  EVP_PKEY_free(tmp_ec_key_pair);

  return std::string((char*)encrypted, ciphertext_len) + tmp_ec_key_pair_str + std::string((char*)iv, 12) + std::string((char*)tag, 16);
 
}

std::string decrypt_response(const std::string& data, const std::string& output_key) {
    uint8_t tag[16];
    ...
    if(EVP_DecryptInit_ex(ctx, EVP_aes_128_gcm(), NULL, reinterpret_cast<const unsigned char*>(output_key.c_str()), iv)!=1) {
...
class SGXTaskClient : public HttpClient {
 public:
  SGXTaskClient(const std::string& addr):
    HttpClient(),
    addr_(addr) {}
  ...
  std::string exec(const std::string& input, const std::string& output_key) {
    ...
    // encrypt the input
    std::string encrypted_input = encrypt_request(input, enclave_key_, output_key);
    ...
    json_request["value"] = hex_encode(encrypted_input.data(), encrypted_input.size());
    ...
    std::string decrypted_response = decrypt_response(hex_decode(msg.c_str(), msg.size()), output_key);
...
int main() {
...

  {
    ...
    client.ra();
    auto response = client.exec("sbip", std::string((char*)default_output_key, 16));
    std::cout << "Response: " << response << "\n";
  }
