...
std::string get_key_and_report() {
  ...
  sgx_status_t status =
      generate_key_pair(RSA_TYPE, &public_key_buffer, &public_key_size,
...
std::string decrypt_content(const char* input_data, size_t input_data_size,
  ...
  const char* ek = input_data + (input_data_size - MAX_SESSION_KEY_IV_LENGTH -
                                 ENCRYPTED_SESSION_KEY_LENGTH);
  int encrypted_input_len = input_data_size - (MAX_SESSION_KEY_IV_LENGTH +
                                               ENCRYPTED_SESSION_KEY_LENGTH);
  // perform decryption
  int decrypted_len;
  unsigned char* decrypted = rsa_decrypt_data(
      (unsigned char*)input_data, encrypted_input_len, (unsigned char*)ek,
      ENCRYPTED_SESSION_KEY_LENGTH, (unsigned char*)iv, enclave_private_key,
      enclave_private_key_size, &decrypted_len);
  printf("decrypted (%dB):\n", decrypted_len);
...
void encrypt_output(const std::string& output_plain,
...
int main(int argc, char* argv[]) {
  ...
  // session key(256B)|IV(16)| the secret data should consists of |input|output
  // key(16B)|
