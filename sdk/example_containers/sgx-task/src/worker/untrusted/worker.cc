...
std::string Worker::GetKeyAndReport(std::string* retKey) {
  ...
  sgx_status_t status = enc_get_key_and_report(
      eid_, &retval, &qe_target_info, &app_report, pubkey, PUBKEY_SIZE);
...
int Worker::Execute(const std::string& request, std::string* output) {
  ...
  std::cout << "exec started at: "
            << std::chrono::system_clock::now().time_since_epoch() /
                   std::chrono::microseconds(1)
            << "\n";
  enc_run(eid_, &retval, request.data(), request.size(), &output_size);
