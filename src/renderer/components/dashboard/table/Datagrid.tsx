...
const Datagrid: React.FC<DatagridProps> = ({ data, isTableExpanded, setIsTableExpanded, propConfigList }) => {
  ...
  const [orderBy, setOrderBy] = useState<keyof InternalDataEntry | keyof ExternalDataEntry>('session_start_datetime');
  ...
  const [maxTableHeight, setMaxTableHeight] = useState(maxRowHeight * (unexpandedRowPerPage + 1) + unexpandedRowPerPage - 1)
  ...
  const visibleRows = useMemo(() =>
      stableSort<InternalDataEntry | ExternalDataEntry>(data, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
  ...
  useEffect(()=>{
    if (isTableExpanded) {
      // setRowsPerPage(expandedRowPerPage);
      // setTimeout(()=>{
        setTableOverflow(true);
        setAddRightPadding(false);
      // }, 500)
      setMaxTableHeight(maxRowHeight * (expandedRowPerPage + 1) + expandedRowPerPage - 1);
    } else {
      setMaxTableHeight(maxRowHeight * (expandedRowPerPage + 1) + expandedRowPerPage - 1);
        // setRowsPerPage(unexpandedRowPerPage);
      // setTimeout(()=>{
    ...
    >
        ...
        <TableContainer id='tablecontainer' 
          sx={{
            ...
            overflow: tableOverflow ? "none" : "hidden"
          }}
        >
          <Table
            sx={{ 
