...
const TitleTypography : FC<TitleTypographyProps> = ({title}) => {
    ...
    return (
        <Typography color={theme.palette.cerulean.main} letterSpacing="0.05em" fontSize="16px" sx={{margin: "1rem 1rem 1rem 1rem"}}>{title}</Typography>  
    )
}
const DataTypography : FC<DataTypographyProps> = ({data}) => {
    return (
        <Typography fontSize="16px" sx={{margin: "1rem 1rem 1rem 1rem"}}>{data}</Typography>  
    )
}
...
const TransactionDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:3000/providerdata?session_id=${sessionId}`);
        if (!response.ok) {
  ...
  return (
    <Grid container sx={{height: "100%"}}>
        <Box sx={{
            ...
            <Button onClick={()=>navigate('/clientdashboard')} sx={{width:"7rem"}}>
                <ArrowBackIcon style={{fontSize:'16px', marginRight: '0.5rem'}}/>
                <Typography variant="h3" fontSize="15px" paddingTop='2px'>BACK</Typography>
            </Button>
        </Box>
        <Grid container item xs={12} sx={{height: "7%", justifyContent: 'left', alignItems: 'center', padding: "0 0 0 2.5rem"}}>
          <Typography variant="h1" fontSize="20px">TRANSACTION #</Typography><Typography color={theme.palette.cerulean.main} fontSize="20px">{data.session_id}</Typography>  
        </Grid>
        <Grid container item xs={12} sx={{
            ...
            <Grid item xs={7.5}>
                ...
                <DataTypography data={convertEpochToStandardTimeWithDate(data.session_start_datetime)}/>
                <DataTypography data={convertEpochToStandardTimeWithDate(data.session_end_datetime)}/>
                <DataTypography data={limitDecimalPlaces(data.duration/60/60, 2) + ' hours'}/>
                <DataTypography data={limitDecimalPlaces(randomNormalDistribution(85, 10), 2) + ' %'}/>
                <DataTypography data={data.task}/>
                <DataTypography data={data.is_host ? "Host" : "Client"}/>
                <DataTypography data={limitDecimalPlaces(data.resource_consumed*10, 2) + ' compute credits'}/>
                <DataTypography data={limitDecimalPlaces(data.resource_consumed*3*0.4334, 2) + ' SGD'}/>
